...
const discoveredDevices = Storage.create();
discoveredDevices.init();
const registeredDevices = Storage.create();
...
function getRegisteredDevice(deviceId) {
    return registeredDevices.getDataValue(deviceId);
}
    ...
    Bravia.discover(timeout)
        .then(devices => {
            for (let device in devices) {
                ...
                let registeredDevice = registeredDevices.getDataValue(deviceId);
                if(registeredDevice != null && registeredDevice.host != devices[device].host) {
                    registeredDevice.host = devices[device].host;
                    registeredDevices.setItemSync(registeredDevice);
                }
                discoveredDevices.set(deviceId, devices[device]);
            }
        })
        .catch(error => console.error(error));
}

module.exports.initDeviceDiscovering = function() {
...
module.exports.braviaButtonPressed = function braviaButtonPressed(name, deviceid) {
    ...
    const braviaDevice = getRegisteredDevice(deviceid);
...
module.exports.discoveredDevices = function() {
    let discovered = [];
    discoveredDevices.forEach(function(key, value) {
        discovered.push({id: key, name: asDeviceName(value), reachable: true});
    });
    return discovered;
};
...
module.exports.registerDevice = function (deviceId) {
    ...
    let device = discoveredDevices.getDataValue(deviceId,);
...
module.exports.deRegisterDevice = function (deviceId) {
    registeredDevices.removeItemSync(deviceId);
};

module.exports.getDiscoveredDevice = function(deviceId) {
    return discoveredDevices.getDataValue(deviceId);
};
